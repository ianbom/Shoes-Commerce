        ...
        <footer className="bg-[#111111] text-white">
            <section>
                <div className="mx-auto grid max-w-[1440px] gap-9 px-5 py-8 sm:px-8 md:grid-cols-2 lg:grid-cols-[1.4fr_0.7fr_0.7fr_0.7fr_1fr] lg:px-20">
                    <section>
                        <Link href="/" className="text-[30px] leading-none font-black tracking-[-0.05em] text-white uppercase">
                            NEXSTEP
                        </Link>
                        <p className="mt-4 max-w-[270px] text-[13px] leading-5 font-medium text-white/68">
                            Premium sneakers, limited drops, and streetwear for the culture.
                        </p>
                        <div className="mt-5 flex gap-3">
                            {[Instagram, Facebook, Youtube].map((Icon, index) => (
                                <Link key={index} href="/" aria-label={`Social link ${index + 1}`} className="inline-flex h-8 w-8 items-center justify-center text-white/76 hover:text-primary">
                    ...
                    {columns.map((column) => (
                        <section key={column.title}>
                            <h3 className="text-[13px] font-extrabold text-white">{column.title}</h3>
                            <ul className="mt-4 grid gap-2">
                                {column.links.map(([label, href]) => (
                                    <li key={label}>
                                        <Link href={href} className="text-[13px] font-medium text-white/68 hover:text-primary">
                                            {label}
                                        </Link>
                                    </li>
                                ))}
                    ...
                    <section>
                        <h3 className="text-[13px] font-extrabold text-white">We Accept</h3>
                        <div className="mt-4 flex flex-wrap gap-2">
                            {payments.map((payment) => (
                                <span key={payment} className="rounded-[3px] bg-white px-2 py-1 text-[11px] font-black text-ink">
                                    {payment}
                                </span>
                            ))}
                        </div>
                        <Link href="/list" className="mt-6 inline-flex items-center gap-2 text-[13px] font-bold text-white hover:text-primary">
